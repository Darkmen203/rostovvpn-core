...
const defaultTunInterfaceName = "RostovVPNTunnel"

func DefaultTunInterfaceName() string {
	if runtime.GOOS == "darwin" {
		return ""
	}
	return defaultTunInterfaceName
}
...
func normalizeDNSAddress(addr string) string {
	...
	host := u.Host
	hasPort := strings.LastIndex(host, ":") > strings.LastIndex(host, "]") // IPv6 with []
	if !hasPort {
		switch {
...
func setInbound(options *option.Options, opt *RostovVPNOptions) {
	...
	mixedOptions := &option.HTTPMixedInboundOptions{
		ListenOptions: option.ListenOptions{
			...
			InboundOptions: option.InboundOptions{
				SniffEnabled:             true,
				SniffOverrideDestination: false, // ?????? true
				DomainStrategy:           inboundDomainStrategy,
			},
		},
...
func warnIfTLSTricksRequestedButUnsupported(opt *RostovVPNOptions) {
	if hasTLSTricks(opt) {
		log.Println("[dns] TLS tricks (fragment/padding/mixed_sni_case) запрошены," +
